...
import { 
  ...
  Loader2, DollarSign, Bot as BotIcon, ChevronRight
} from 'lucide-react';
    ...
    return (
        <div className="space-y-12 animate-in fade-in">
            ...
            <div className="bg-slate-900/20 border border-white/5 rounded-[44px] p-10">
                <h3 className="text-sm font-black text-white italic uppercase tracking-widest mb-8">Motor Durumu</h3>
                <div className="py-12 border-2 border-dashed border-slate-800 rounded-3xl text-center">
                    <p className="text-slate-600 text-[10px] font-bold uppercase italic animate-pulse">Botly Engine aktif ve beklemede...</p>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-8 animate-in fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                <div>
                    <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter leading-none">Reklam <span className="text-blue-500">Forge</span></h2>
                    <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mt-2 italic">Ağ üzerinden toplu mesaj dağıtımı</p>
                </div>
                <button onClick={() => { setEditingPromo({ title: '', content: '', status: 'pending', button_text: 'İNCELE', processed_channels: [] }); setIsModalOpen(true); }} className="bg-emerald-600 px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 border-b-4 border-emerald-800">YENİ REKLAM FORGE ET</button>
            </div>

            <div className="grid grid-cols-1 gap-6">
                {isLoading ? <div className="flex justify-center py-24"><Loader2 className="animate-spin text-blue-500" /></div> : promos.map(p => (
                    <div key={p.id} className="bg-slate-900/40 border border-white/5 rounded-[40px] p-8 flex flex-col md:flex-row items-center justify-between group shadow-2xl">
                        <div className="flex-1">
                            ...
                            <h4 className="text-xl font-black italic uppercase text-white truncate">{p.title}</h4>
                            <p className="text-[10px] text-slate-500 font-bold uppercase italic mt-1">{p.channel_count || 0} Kanal Hedeflendi</p>
                        </div>
                        <div className="flex gap-2 mt-4 md:mt-0">
                            <button 
                                ...
                                className={`px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${p.status === 'sending' ? 'bg-orange-500/10 text-orange-500 border border-orange-500/20' : 'bg-blue-600 text-white shadow-lg shadow-blue-900/20'}`}
                            >
                                {p.status === 'sending' ? 'DURDUR' : 'BAŞLAT'}
                            </button>
                            <button onClick={async () => { if(confirm('Silmek istediğine emin misin?')) { await DatabaseService.deletePromotion(p.id); load(); } }} className="p-4 bg-white/5 rounded-xl text-red-500 hover:bg-red-600 hover:text-white transition-all"><Trash2 size={18}/></button>
                        </div>
                    </div>
                ))}
            </div>
            ...
            {isModalOpen && editingPromo && (
                <div className="fixed inset-0 z-[110] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl animate-in fade-in">
                    <div className="bg-[#020617] p-10 rounded-[64px] w-full max-w-2xl border border-white/10 shadow-2xl relative">
                        ...
                        <h3 className="text-2xl font-black mb-8 uppercase italic tracking-tighter">İçerik <span className="text-emerald-500">Forge</span></h3>
                        <form onSubmit={async (e) => { 
                            ...
                            <AdminInput label="BAŞLIK" value={editingPromo.title} onChange={(v:any)=>setEditingPromo({...editingPromo, title:v})} placeholder="Reklam başlığı..." />
                            <div className="space-y-2">
                                <label className="text-[9px] font-black text-slate-700 uppercase tracking-widest ml-4 italic">MESAJ İÇERİĞİ</label>
                                <textarea value={editingPromo.content} onChange={e => setEditingPromo({...editingPromo, content: e.target.value})} className="w-full bg-slate-950 border border-white/5 p-6 rounded-[32px] text-[11px] font-black h-32 outline-none text-slate-400 focus:border-emerald-500/30 uppercase italic" placeholder="Botlar aracılığıyla gönderilecek mesaj..." />
                            </div>
                            ...
                            <button type="submit" className="w-full h-16 bg-emerald-600 rounded-[32px] font-black text-[11px] uppercase tracking-widest border-b-4 border-emerald-800 shadow-xl active:scale-95">REKLAMI FORGE ET</button>
                        </form>
                    </div>
                </div>
            )}
...
const AdminDashboard = () => {
    ...
    const menu = [
        ...
        { path: '/a/dashboard/settings', icon: Settings, label: 'Ayarlar' }
    ];
