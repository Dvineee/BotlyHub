...
const MyChannels = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 px-5 sm:px-8 pt-6 md:pt-10 pb-32 animate-in fade-in transition-colors duration-300" onClick={() => setOpenSettingsId(null)}>
      ...
      <div className="flex items-center justify-between mb-10 px-1">
        ...
        <div className="flex items-center gap-2">
            ...
            <button 
                ...
                disabled={isSyncing}
                className="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-900/80 border border-black/5 dark:border-white/5 rounded-xl text-slate-500 dark:text-slate-400 active:scale-90 transition-all disabled:opacity-30 "
            >
      ...
      {autoSyncStatus !== 'idle' && (
          <div className={`mb-6 p-4 rounded-xl border flex items-center justify-center gap-3 animate-in slide-in-from-top-4 transition-all ${
      ...
      {isLoading ? (
          ...
          <div className="text-center py-20 bg-white dark:bg-slate-900/20 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-900 flex flex-col items-center gap-6 ">
              <div className="w-20 h-20 bg-slate-50 dark:bg-slate-900 rounded-xl flex items-center justify-center text-slate-200 dark:text-slate-800"><Megaphone size={40} /></div>
              <div className="px-10">
              ...
              <button 
                onClick={(e) => { e.stopPropagation(); handleManualRefresh(); }} 
                className="px-8 py-4 bg-slate-900 border border-slate-800 rounded-xl text-[10px] font-black text-purple-500 dark:text-blue-500 uppercase tracking-widest hover:text-white transition-all active:scale-95"
              >
                  Şimdi Tara
              </button>
          </div>
          ...
          <div className="space-y-4">
              ...
              {channels.map(c => (
                  <div key={c.id} className="group relative bg-white dark:bg-slate-900/40 border border-black/5 dark:border-white/5 rounded-xl overflow-hidden transition-all hover:bg-slate-50 dark:hover:bg-slate-900/60  animate-in slide-in-from-bottom-2">
                      <div className="p-5 flex items-center h-full pr-14 relative z-10">
                          <div className="relative shrink-0">
                            <img 
                                src={c.icon} 
                                className="w-16 h-16 rounded-xl border border-black/5 dark:border-white/5  object-cover bg-slate-50 dark:bg-slate-900" 
                                onError={(e) => {
                      ...
                      <div className={`absolute inset-0 z-30 transition-transform duration-300 ease-out flex items-center justify-center p-3 ${openSettingsId === c.id ? 'translate-x-0' : 'translate-x-full'}`}>
                           ...
                           <div className="w-full h-full flex items-center justify-center gap-4 relative z-40">
                                <div 
                                    ...
                                    className={`cursor-pointer rounded-xl px-6 py-4 flex items-center gap-4 border transition-all active:scale-95  ${
                                ...
                                <div className="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-900/80 rounded-xl text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer border border-black/5 dark:border-white/5 " onClick={(e) => { e.stopPropagation(); setOpenSettingsId(null); }}>
                                    <X size={22} />
                                </div>
                           </div>
                      </div>
      ...
      <div className="mt-10 p-6 bg-purple-600/5 border border-purple-600/10 rounded-xl ">
