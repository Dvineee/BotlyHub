    ...
    <div onClick={handleClick} className="flex items-center py-4 px-3 cursor-pointer group hover:bg-slate-900/60 rounded-2xl transition-all border border-transparent hover:border-slate-800/50 mb-1">
        ...
        <div className="flex-1 ml-4 min-w-0 mr-2">
            <div className="flex items-center gap-2 mb-0.5">
                ...
                {bot.price > 0 && <span className="bg-indigo-500/10 text-indigo-400 text-[9px] font-bold px-1.5 py-0.5 rounded border border-indigo-500/20 uppercase tracking-tighter">{t('premium')}</span>}
            </div>
            ...
            <div className="relative" ref={menuRef}>
                ...
                {isMenuOpen && (
                    <div className="absolute right-0 top-full mt-3 w-52 bg-[#0f172a] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden z-[100] animate-in fade-in">
                        <button onClick={() => { navigate('/'); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-slate-800 text-left border-b border-slate-800/50"><Store size={18} className="text-blue-400" /> <span className="text-sm font-bold">{t('market')}</span></button>
                        <button onClick={() => { navigate('/settings'); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-slate-800 text-left border-b border-slate-800/50"><User size={18} className="text-purple-400" /> <span className="text-sm font-bold">{t('profile')}</span></button>
                        <button onClick={() => { navigate('/my-bots'); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-slate-800 text-left border-b border-slate-800/50"><BotIcon size={18} className="text-emerald-400" /> <span className="text-sm font-bold">{t('my_bots')}</span></button>
                        <button onClick={() => { navigate('/channels'); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-slate-800 text-left"><Megaphone size={18} className="text-orange-400" /> <span className="text-sm font-bold">{t('my_channels')}</span></button>
                    </div>
                )}
            </div>
        </div>
            ...
            <div className="space-y-1">
                <div className="flex justify-between items-center mb-6 px-1">
                    <h2 className="text-lg font-black text-white tracking-tight uppercase">
                        {searchQuery ? t('search_results') : (activeCategory === 'all' ? t('featured') : t(categories.find(c => c.id === activeCategory)?.label || ''))}
                    </h2>
                </div>
                {filteredBots.length > 0 ? (
                    ...
                    <div className="py-20 text-center flex flex-col items-center gap-4">
                        <p className="text-slate-600 font-bold italic">{t('no_results')}</p>
                    </div>
                )}
            </div>
          </>
      ...
      {selectedAnn && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm animate-in fade-in" onClick={() => setSelectedAnn(null)}>
            <div className="bg-[#0f172a] w-full max-w-[280px] rounded-[24px] overflow-hidden shadow-2xl relative" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={() => setSelectedAnn(null)} className="absolute top-4 right-4 w-8 h-8 bg-slate-800/40 rounded-full flex items-center justify-center text-slate-500 hover:text-white transition-colors"><X size={16}/></button>
