    ...
    <div className="home-search-bar latest-slider-container mb-6 md:mb-10 flex flex-col md:flex-row items-center !gap-[0.3rem] bg-[#ffffff] dark:bg-[#1e293b] px-4 md:px-[10px] !pt-[0.3rem] !pb-0 -mx-4 md:mx-0 rounded-none md:rounded-xl border-y md:border border-black/5 dark:border-white/5 relative overflow-hidden group !shadow-none">
      ...
      <div className="flex flex-col shrink-0 min-w-full md:min-w-[180px] md:border-r border-black/5 dark:border-white/5 md:pr-6 h-full justify-center">
        <div
          ...
          onClick={() => {
            if (window.innerWidth < 768) cycleType();
          }}
        >
        ...
        <AnimatePresence>
          {scrollState.left && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-slate-50 dark:from-slate-950 via-slate-50/80 dark:via-slate-950/80 to-transparent z-40 pointer-events-none flex items-center pl-2"
            >
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  scroll.ref.current?.scrollBy({
                    left: -300,
                    behavior: "smooth",
                  });
                }}
        ...
        <AnimatePresence>
          {scrollState.right && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-slate-50 dark:from-slate-950 via-slate-50/80 dark:via-slate-950/80 to-transparent z-40 pointer-events-none flex items-center justify-end pr-2"
            >
              <button
                onClick={(e) => {
                  e.stopPropagation();
...
const BotCard: React.FC<{ bot: Bot; tonRate: number; featuredRank?: number }> =
...
const CategoryBotCard: React.FC<{ bot: Bot; rank: number }> = React.memo(
  ({ bot, rank }) => {
    const navigate = useNavigate();
    return (
      <div
...
const Home = () => {
  ...
  const [announcements, setAnnouncements] =
    useState<Announcement[]>(visualPromos);
  ...
  const filteredBots = useMemo(() => {
    ...
    else if (activeFilter === "bhub")
      result = result.filter((b) => b.is_official);
  ...
  const loadData = useCallback(async () => {
    ...
    setBots(botData);
    const activeAnns = annData.filter(
      (a) => a.is_active && !isDemoAnnouncement(a),
    );
    const combinedAnns = [
      ...visualPromos,
      ...activeAnns.filter(
        (dbAnn) =>
          !visualPromos.some(
            (vp) =>
              vp.title.toLowerCase() === dbAnn.title.toLowerCase() ||
              vp.id === dbAnn.id,
          ),
      ),
    ];
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !homeSearchRef.current.contains(event.target as Node)
      ) {
  ...
  const categorizedBots = useMemo(() => {
    const result: Record<
      string,
      { featured: Bot[]; slider: Bot[]; total: number }
    > = {};
    const baseBots = [...filteredBots].sort(
      (a, b) => (b.views || 0) - (a.views || 0),
    );

    // Apps section
    const allApps = baseBots.filter((b) =>
    ...
    if (selectedAppsCategory !== "all") {
      const appsCategoryMap: Record<string, (b: Bot) => boolean> = {
        ...
        web3_general: (b) =>
          Array.isArray(b.category)
            ? b.category.includes("crypto") || b.category.includes("finance")
            : b.category === "crypto",
        tma_bots: (b) => true,
        ton_sites: (b) => true,
        saas: (b) =>
    ...
    const allBots = baseBots.filter((b) =>
      Array.isArray(b.category)
        ? !b.category.includes("apps")
        : b.category !== "apps",
    );
    ...
    if (openMenu === "kesfet") {
      return (
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-12 gap-8">
          {/* Left Column (Promotion Panel) */}
          <div className="col-span-4 bg-[#f1f6ff] dark:bg-slate-800/20 border border-[#e2edff] dark:border-slate-800/40 rounded-2xl p-5 flex flex-col justify-between select-none">
            {/* Top section */}
            <div className="flex flex-col gap-3">
              <h4 className="text-[13px] font-bold text-slate-800 dark:text-slate-200 leading-snug">
                Telegram kanalınızın amacına göre en uygun botları bir arada inceleyin.
              ...
              <div className="flex flex-wrap gap-2 my-1">
                {/* Red Target Icon */}
                <span className="w-7 h-7 rounded-full bg-[#ff4d4f] flex items-center justify-center shadow-sm select-none shrink-0">
                  <span className="w-3 h-3 rounded-full bg-white flex items-center justify-center">
                    <span className="w-1 h-1 rounded-full bg-[#ff4d4f]"></span>
                  </span>
                </span>
                ...
                {/* White/Blue border circle with blue square inside */}
                <span className="w-7 h-7 rounded-full bg-white dark:bg-slate-900 border border-blue-500/80 flex items-center justify-center shadow-sm shrink-0">
                  <span className="w-2.5 h-2.5 bg-blue-600 rounded-sm"></span>
                </span>

                {/* Yellow/Black hexagonal pattern */}
                <span className="w-7 h-7 rounded-full bg-amber-500 flex items-center justify-center shadow-sm shrink-0">
                  <span className="text-[9px] font-extrabold text-slate-950">✦</span>
                </span>

                {/* Mint/Dark green */}
                <span className="w-7 h-7 rounded-full bg-[#181d20] flex items-center justify-center shadow-sm shrink-0">
                  <span className="w-3.5 h-2.5 rounded-full border-2 border-emerald-400 flex items-center justify-center">
                    <span className="w-1 h-1 rounded-full bg-emerald-400"></span>
                  </span>
                </span>
                ...
                {/* Dark Black with italic T */}
                <span className="w-7 h-7 rounded-full bg-black flex items-center justify-center shadow-sm shrink-0">
                  <span className="text-[11px] font-[900] italic text-white font-serif">T</span>
                </span>

                {/* Light blue/grey with circle-A */}
                <span className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 border border-dashed border-slate-300 dark:border-slate-700 flex items-center justify-center shadow-sm shrink-0">
                  <span className="text-[9px] font-black text-slate-500 dark:text-slate-400">A</span>
                </span>

                {/* Black with white slash/stripes */}
                <span className="w-7 h-7 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center shadow-sm shrink-0">
                  <span className="text-[10px] font-bold text-white leading-none">☰</span>
                </span>

                {/* Feather with dotted boundary */}
                <span className="w-7 h-7 rounded-full bg-white dark:bg-slate-900 border border-dotted border-slate-400 dark:border-slate-700 flex items-center justify-center shadow-sm shrink-0">
                  <span className="text-[10px]">🪶</span>
                </span>
              </div>
              ...
              <button
                ...
                className="text-[12px] font-extrabold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1.5 mt-1 cursor-pointer transition-colors self-start"
              >
                Şimdi dene <ArrowRight size={13} className="shrink-0" />
              </button>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                className="text-[12px] font-extrabold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1.5 mt-1 cursor-pointer transition-colors self-start"
              >
                Daha fazla bilgi edin <ArrowRight size={13} className="shrink-0" />
              </button>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-200 animate-in transition-colors duration-300 home-page">
      <SEO title={t("home_seo_title")} description={t("home_seo_desc")} />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="hidden"
        style={{ display: "none" }}
      >
        <symbol id="solar.fire" viewBox="0 0 24 24">
          <g fill="currentColor">
            ...
            <path
              d="M11.6 9C11.6 9 10.2 10.2 9.4 11.7C8.6 13.2 8.1 15 8.1 16.5C8.1 19.5 10.5 22 13.6 22C16.7 22 19.1 19.5 19.1 16.5C19.1 15 18.6 13.2 17.8 11.7C17 10.2 15.6 9 15.6 9C15.6 11 14.5 12.5 13.1 13C11.8 13.5 11.6 10.5 11.6 9Z"
              opacity="0.6"
            />
          </g>
        </symbol>
                      ...
                      <div className="hidden md:block">
                        ...
                        <>
                          ...
                          <button
                            ...
                            className="flex md:hidden w-full flex-center items-center gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400 transition-all group text-left"
                          >
                            <Store
                              size={18}
                              className="text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors"
                            ...
                            <div className="flex flex-col gap-6 overflow-hidden">
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  ...
                                  <ChevronRight
                                    size={18}
                                    className="text-slate-400 dark:text-slate-600 group-hover:translate-x-1 transition-transform"
                                  />
                                </div>
                                ...
                                <>
                                  ...
                                  <div className="sm:hidden relative -mx-4 px-4 overflow-hidden">
                                    <div className="flex gap-4 overflow-x-auto no-scrollbar scroll-smooth snap-x snap-mandatory pb-4">
                                      {getMobileColumns(sliderBots).map(
                                        (col, colIdx) => (
                                          <div
                                            key={colIdx}
                                            className="flex flex-col gap-5 min-w-[85vw] snap-center first:pl-2"
                                          >
                                            {col.map(({ bot, rank }) => (
                                              <CategoryBotCard
                                                key={bot.id}
                              ...
                              <div className="flex items-center gap-2 max-w-[70%]">
                                <div className="w-6 h-6 rounded-full overflow-hidden bg-slate-100 dark:bg-white/5 border border-black/5 dark:border-white/10 shrink-0">
                                  <img
                                    src={
                                      post.authorAvatar ||
                                      `https://ui-avatars.com/api/?name=${encodeURIComponent(post.author)}&background=334155&color=fff&bold=true`
                                    }
                                    alt=""
                  ...
                  <div className="prose prose-slate dark:prose-invert max-w-none">
                    <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight italic mb-6">
                      {t("home_about_title").includes(":") ? (
                        <>
                          {t("home_about_title").split(":")[0]}:{" "}
                          <span className="text-blue-500">
                            {t("home_about_title").split(":")[1]?.trim()}
                          </span>
                        </>
                      ) : (
                        t("home_about_title")
                ...
                <div className="mt-20 mb-20 px-4 max-w-4xl mx-auto">
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <details
                        key={idx}
                        className="group bg-white dark:bg-slate-900/40 border border-black/5 dark:border-white/5 rounded-xl transition-all overflow-hidden shadow-sm"
                      >
                        <summary className="flex items-center justify-between p-5 cursor-pointer list-none">
                          <span className="text-[13px] font-bold text-slate-900 dark:text-white uppercase tracking-tight italic">
                            {item.q}
                          </span>
                          <ChevronDown
                            size={18}
                ...
                {selectedAnn.bg_image_url ? (
                  ...
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${
                      selectedAnn.color_scheme === "purple"
                        ? "from-purple-600 to-indigo-700"
                        : selectedAnn.color_scheme === "emerald"
                          ? "from-emerald-600 to-teal-700"
                          : selectedAnn.color_scheme === "blue"
                ...
                <div className="flex items-center gap-4 mb-4">
                  {selectedAnn.icon_name !== "None" && (
                    <div className="w-12 h-12 bg-slate-100 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-xl flex items-center justify-center shrink-0">
                      {React.createElement(
                        iconMap[selectedAnn.icon_name] || Sparkles,
                        {
                          size: 22,
