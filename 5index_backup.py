...
async def get_telegram_stats(username, message_id):
    ...
    # 2 defa tekrar dene (Rate limit vb. durumlara karşı)
        ...
        async with httpx.AsyncClient(timeout=15.0, headers=headers) as client:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logger.warning(f"⚠️ [429] Rate Limit ({username}/{message_id}) - Bekleniyor...")
                    await asyncio.sleep(2)
                else:
                    logger.warning(f"⚠️ Scrape HTTP {resp.status_code} ({username}/{message_id})")
                    break # Diğer durumlarda tekrar deneme
        ...
        try:
            ...
            for p in promos:
                ...
                if message_map:
                    ...
                    if source_username:
                        sem = asyncio.Semaphore(5) # Rate limit koruması için aynı anda maksimum 5 paralel istek
                        
                        async def fetch_channel_stats(target_tg_id, source_msg_id):
                            async with sem:
                                try:
                        ...
                        # Tüm kanallar için aynı anda (paralel) görev oluştur
                        tasks = [fetch_channel_stats(tgt, src) for tgt, src in message_map.items()]
                        results = await asyncio.gather(*tasks)
                        
                        # Dönen sonuçları ana toplama ekle
                        for v, r in results:
                            total_views += v
                            total_reactions += r
                            
                        # Promo geçişleri arasına bekleme (Her reklam tamamlandığında)
